...
同一の影響レベルに分類された組織システムの中で、優先順位をつける作業である。  
...
**期待される出力**：組織システムが、低・中・高の影響レベルごとのサブカテゴリに優先順位づけされる  
...
**支援役**：機関の上級情報セキュリティ責任者、プライバシー担当上級機関責任者、ミッションまたは事業責任者、システム所有者、最高情報責任者、承認責任者またはその指定代理人

**Discussion**:  
This task is carried out only after organizational systems have been categorized (see Task C1).   

**議論**：
このタスクは、まず組織の情報システムが分類された後（タスクC1参照）に実施される。
...
この段階では、[FIPS 199] および [FIPS 200] に基づいて分類された各システムに対し、「ハイウォーターマーク（最も高い評価）」の原則を適用する。[^62] この適用により、各システムは「低影響」「中影響」「高影響」に分類される。  
リスクベースの意思決定において、さらに細かな影響レベルで判断したい組織は、このタスクを使って、同一の影響レベル内でシステムに優先順位をつけることができる。[^63]  
たとえば、ある組織は中影響レベルのシステムを、以下の3つに分類する方針を取ることがある：

- 低・中影響システム
- 中・中影響システム
- 高・中影響システム

この場合、高・中影響システムは中・中影響システムより優先度が高く、低・中影響システムはその逆となる。  
このように分類することで、組織は識別されたリスクに対応する際、コントロール選定やベースライン調整について、より合理的な判断が可能となる。  
...
影響レベルによる優先順位付けは、ミッションや業務運用にとって重要または中核的なシステムを特定する目的にも使える。その際は、複雑性、データや機能の集中度、他システムとの接続関係といった要素に着目する。たとえば、影響が「高」に分類されたシステム群をさらに細分化し、「低・高影響」「中・高影響」「高・高影響」などに優先順位をつけることで、特に重要なシステムが明確になる。  
この優先順位付けは組織のあらゆる階層で実施でき、システム所有者が報告したセキュリティ分類データを基に判断される。また、より詳細な影響レベルに応じたコントロール群を定める必要がある場合、組織に特化したベースラインの作成が求められる。  
...
サイバーセキュリティフレームワーク・プロファイルも、このタスクを支援する上で有用である。プロファイルに定義されたミッション・事業目標や、優先順位付けされた成果項目は、同一の影響レベルにある複数システムの間で、どれを優先すべきかを判断する手がかりになる。プロファイルは、組織のミッション・事業目標の重要度に応じて構成されており、各目標に相対的な優先度が割り当てられる。  
たとえば、あるプロファイルにおいて「人的安全」と「環境保護」が最優先の目標に設定されている場合、タスクP-6の実施においては、人的安全に関わるシステムが、同じ影響レベルでも他の目的に関わるシステムよりも高い優先順位を持つことになる。  
...
**参考文献**： [FIPS 199]、[FIPS 200]、[SP 800-30]、[SP 800-39]（組織およびシステムレベル）、[SP 800-59]、[SP 800-60 v1]、[SP 800-60 v2]、[SP 800-160 v1]（システム要件定義プロセス）、[IR 8179]（重要度分析プロセスB）、[CNSSI 1253]、[NIST CSF]（コア：Identify機能、プロファイル）  
...
コントロールの有効性を継続的にモニタリングするための組織全体の戦略を策定し、実施する。  
...
**潜在的な入力**：リスク管理戦略、組織およびシステムレベルのリスク評価結果、組織のセキュリティおよびプライバシーポリシー  

**Expected Outputs**: An implemented organizational continuous monitoring strategy.  

**期待される成果**：実施された組織全体の継続的モニタリング戦略  
...
**支援役**：最高情報責任者、上級機関情報セキュリティ責任者、上級機関プライバシー責任者、ミッションまたは事業責任者、システム責任者、承認責任者または承認責任者指定代理人  
...
**議論**：リスク管理の重要な側面の一つは、組織全体のセキュリティおよびプライバシーの態勢、ならびに組織システム内で実装または継承された制御の有効性を継続的に監視する能力だ。64 組織全体の継続的モニタリング戦略を効果的に実施することは、このようなモニタリングを効率的かつコスト効果の高い方法で実施するために不可欠だ。継続的監視戦略には、サプライチェーンのリスク考慮事項も含まれる場合があります。例えば、サプライヤーの外国所有、支配、または影響力（FOCI）の定期的なレビュー、在庫予測の監視、またはサプライヤーに対する継続的な監査の要求などです。堅牢で包括的な継続的モニタリングプログラムの実施は、組織が情報システムのセキュリティおよびプライバシーの状況を把握するのに役立つ。また、最初のシステムまたは共通統制の承認後の継続的な承認も容易になる。これには、ミッションや業務機能、利害関係者、技術、脆弱性、脅威、リスク、およびシステム、コンポーネント、またはサービスのサプライヤーの変更の可能性も含まれる。  
...
組織の継続的モニタリング戦略は、組織、ミッション/ビジネスプロセス、および情報システムレベルでのモニタリング要件に対処する。継続的モニタリング戦略は、組織全体で実施されるコントロールの最低モニタリング頻度を特定し、継続的なコントロール評価アプローチを定義し、継続的な評価の実施方法（自動化ツールの使用と管理、およびモニタリングを自動化できないコントロールの継続的な評価に関する指示など）を記述する。継続的モニタリング戦略では、報告書の受領者を含む、セキュリティおよびプライバシーに関する報告要件も定義することができる。コントロールのモニタリングの最低頻度を決定するための基準は、組織の役員（リスク管理またはリスク担当上級役員（機能）、機関の上級情報セキュリティ責任者、プライバシー担当上級役員、最高情報責任者、システム所有者、共通コントロールプロバイダー、および承認役員またはその指定代理人など）と協力して確立する。  
...
組織のリスク評価は、モニタリングの頻度を決定するための指針および情報源として利用することができる。自動化を利用することで、モニタリングプロセスの一環として、制御評価の頻度と量を増加させることができる。自動化ツールおよびサポートデータベースを利用した制御の継続的なモニタリングは、情報システムのほぼリアルタイムのリスク管理を促進し、継続的な承認とリソースの効率的な利用を支援する。リスク管理に関する最高責任者またはリスク執行者（機能）は、コントロールの監視の最低頻度を含む継続的な監視戦略を承認する。  
...
**参考文献**：[SP 800-30]；[SP 800-39]（組織、ミッションまたはビジネスプロセス、システムレベル）；[SP 800-53]； [SP 800-53A]; [SP 800-137]; [SP 800-161]; [IR 8011 v1]; [IR 8062]; [NIST CSF]（コア[識別、検出機能]）；[CNSSI 1253]  
