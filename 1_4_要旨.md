...
公共部門および民間部門（米国の重要インフラを含む）において、ハードウェア、ソフトウェア、ファームウェア、システムの複雑性が著しく増大しており、それに伴って攻撃対象領域（アタックサーフェス）も大きく拡張されている。これは、攻撃者にとって悪用の機会が増加していることを意味する。加えて、敵対者はサプライチェーンを攻撃経路（アタックベクトル）として活用し、システムへの侵入、構成要素の完全性の侵害、さらには重要資産へのアクセスを図る有効な手段として利用している。
...
> 「…タスクフォースは、米国の重要インフラに対するサイバー脅威が、広範に存在する脆弱性の軽減努力を上回っていると指摘している。したがって、少なくとも今後10年間は、米国が最も高度な能力を有する敵対者によるサイバー脅威に対応するため、抑止力に大きく依存せざるを得ない。米国のサイバー抑止に関しては、より積極的かつ体系的なアプローチが緊急に求められていることは明白である…」

このような状況を踏まえ、重要インフラのあらゆる分野において依存している基盤的な情報システム、構成製品、サービスの信頼性をさらに強化することが喫緊の課題となっている。これらのシステム、製品、サービスは、システム開発ライフサイクル（SDLC）を通じて十分な信頼性を備え、米国の経済的および国家安全保障上の利益を支えるために必要な回復力（レジリエンス）を提供できるものでなければならない。
